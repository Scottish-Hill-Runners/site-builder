...
export interface InfoPageEditDialogProps {
  open: boolean;
  onClose: () => void;
  slug: string;
  title: string;
...
export default function InfoPageEditDialog({
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    const subject = `Info page update for ${title} (${slug})`;
    // Info pages may have frontmatter (e.g. title), so only the body is sent.
    const emailBody =
      `To ${toWhomItMayConcern()}:\n\n` +
      `I would like to update the text for this info page.\n\n` +
